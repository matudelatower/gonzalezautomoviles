		...
		if ( is_array( $resultSet ) && ! empty( $resultSet ) || ! is_null( $resultSet ) ) {
			foreach ( $resultSet as $entity ) {
				...
				$phpExcelObject->getActiveSheet()->setCellValue( 'E' . $i,
					date( "d-m-Y", strtotime( $entity['facturas_fecha'] ) ) );
				$i ++;
				$contador ++;
			}
		}
	...
	public function buildSheetgetReporteVehiculosEnStock( $resultSet ) {
		...
		if ( is_array( $resultSet ) && ! empty( $resultSet ) || ! is_null( $resultSet ) ) {
			...
			foreach ( $resultSet as $entity ) {
				...
				$phpExcelObject->getActiveSheet()->setCellValue( 'G' . $i,
					date( 'd-m-Y', strtotime( $entity['fecha_emision_documento'] ) ) );
	...
	public function buildSheetgetReporteAgendaEntregas( $resultSet ) {
		...
		if ( is_array( $resultSet ) && ! empty( $resultSet ) || ! is_null( $resultSet ) ) {
			foreach ( $resultSet as $entity ) {
				...
				$phpExcelObject->getActiveSheet()->setCellValue( 'L' . $i,
					date( "d-m-Y", strtotime( $entity['fecha_entrega'] ) ) . " " . $entity['hora_entrega'] );
	...
	public function buildSheetReporteVehiculosAsignadosAReventa( $resultSet ) {
		...
		if ( is_array( $resultSet ) && ! empty( $resultSet ) || ! is_null( $resultSet ) ) {
			...
			foreach ( $resultSet as $entity ) {
				...
				$phpExcelObject->getActiveSheet()->setCellValue( 'F' . $i,
					$entity['estado_patentamiento'] ? $entity['estado_patentamiento'] : '' );
		...
		if ( is_array( $resultSet ) && ! empty( $resultSet ) || ! is_null( $resultSet ) ) {
			...
			foreach ( $resultSet as $entity ) {
				...
				$phpExcelObject->getActiveSheet()->setCellValue( 'B' . $i,
					$entity['nombre_modelo'] . " | " . $entity['anio_modelo'] . " | " . $entity['version'] );
				$phpExcelObject->getActiveSheet()->setCellValue( 'C' . $i, $entity['color'] );
				$phpExcelObject->getActiveSheet()->setCellValue( 'D' . $i, $entity['vin'] );
...
// create the response
	...
	public function buildSheetgetReporteVehiculosEntregados( $resultSet ) {
		...
		$phpExcelObject->getActiveSheet()
		               ...
		               ->setCellValue( 'F1', 'Tipo de venta' )
		               ->setCellValue( 'G1', 'Fecha entregado' );
